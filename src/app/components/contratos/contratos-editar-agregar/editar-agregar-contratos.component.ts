...
@Component({
    selector: 'editar-agregar-contratos',
    templateUrl: './editar-agregar-contratos.component.html',
    styleUrls: ['./editar-agregar-contratos.component.css'],
...
export class EditarAgregarContratosComponent {
    ...
    private BuildForm() {
        this.addModel = this.fb.group({
            ...
            nombreDealer:new FormControl({value:'', disabled:true}),
        ...
        promise.then((success) => {
            if(this.addModel.get('dealer').value){
                this.serviceDistribution.get(pais.id,this.addModel.get('dealer').value.id).subscribe((response) => {
                    this.addModel.get('puerto').setValue(response.port);
                });
            }
    ...
    selectDealer(){
        ...
        let promise = new Promise((resolved) => {
            ...
            this.addModel.get('nombreDealer').setValue(isSelected ? dealer.name : '');
            resolved(true);
        });
